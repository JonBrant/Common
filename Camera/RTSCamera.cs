...
namespace Com.Pi.RTS {
    ...
    public class RTSCamera : MonoBehaviour {
        ...
        /// </summary>
        public bool FollowingTarget { get { return targetFollow != null; } }
        #endregion
        ...
        private Vector2 KeyboardInput { get { return useKeyboardInput ? new Vector2(Input.GetAxis(horizontalAxis), Input.GetAxis(verticalAxis)) : Vector2.zero; } }
        ...
        private void Start() { m_Transform = transform; }

        private void Update() {
            if (targetFollow && (!KeyboardInput.Equals(Vector3.zero) || (usePanning && Input.GetKey(panningKey)))) {
                if (!KeyboardInput.Equals(Vector3.zero) || !MouseInput.Equals(Vector3.zero)) {
                    targetFollow = null;
                }
            }
        ...
        private void Move() {
            if (MouseOverUI.Value == false) {
                if (useKeyboardInput) {
                    ...
                    desiredMove *= Input.GetKey(KeyCode.LeftShift)?ShiftBoostAmount:1;
                ...
                if (usePanning && Input.GetKey(panningKey) && MouseAxis != Vector2.zero && MouseOverUI.Value == false) {
        ...
        public void SetTarget(Transform target) { targetFollow = target; }
